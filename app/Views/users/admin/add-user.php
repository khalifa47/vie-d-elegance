...
<div id="add-user-bg" style="height: 600px;background: url(&quot;<?= base_url("assets/img/sewing.jpg") ?>&quot;) center / cover no-repeat, linear-gradient(rgba(0,0,0,0.4) 100%, rgba(0,0,0,0.4) 100%, white 100%);">
    <div class="form-box">
        ...
        <div id="apiuser" style="display:none;">
            <input class="form-control inp-field" type="text" placeholder="Username" id="uname" name="uname" required="">
            <input type="hidden" id="admin_id" name="admin_id" value="<?= esc($user['user_id']) ?>">
            <button class="btn btn-primary" type="submit" style="background: rgb(86,198,198);margin-top: 22px;border-radius: 20px;">Register API user</button>
        </div>
        <div id="otheruser" style="display:none;">
            ...
            <button class="btn btn-primary" type="submit" style="background: rgb(86,198,198);margin-top: 22px;border-radius: 20px;">Register</button>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(() => {
        $("#role").on("change", () => {
            const role = $('#role').val();
            if (role == 3) {
                $("#userform").html($("#apiuser").html());
            } else {
                $("#userform").html($("#otheruser").html());
            }
        });
        ...
        $("#add-user-form").on("submit", (e) => {
            ...
            if (role != 3) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url('UsersController/registerAPIuser') ?>',
                    data: {
                        uname: username,
                        admin_id: admin_id
                    },
